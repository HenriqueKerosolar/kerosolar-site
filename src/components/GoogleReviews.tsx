...
// Renderiza as avaliações reais do Google (via Featurable) usando o design do site.
export function GoogleReviews({ featurableId }: { featurableId: string }) {
  return (
      ...
      <div className="container-px">
        ...
        <ReactGoogleReviews
          ...
          renderer={(reviews: ReactGoogleReview[]) => {
            const withText = reviews.filter((r) => r.comment?.trim()).slice(0, 9);
            return (
  ...
  return (
    <figure className="flex flex-col rounded-2xl border border-brand-100 bg-white p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex gap-0.5 text-sun-500" aria-label={`${rating} de 5 estrelas`}>
      ...
      <blockquote className="mt-4 flex-1 text-sm leading-relaxed text-ink/80 line-clamp-6">
        “{review.comment}”
      </blockquote>
...
function Star({ filled }: { filled: boolean }) {
  return (
    <svg width="16" height="16" viewBox="0 0 20 20" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.2" aria-hidden>
